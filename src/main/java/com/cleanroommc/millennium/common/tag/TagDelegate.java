...
public class TagDelegate<T> implements ITaggable<T> {
    private static final HashMap<ITaggable<?>, TagDelegate<?>> DELEGATES = new HashMap<>();
    private static final HashMultimap<Tag, TagDelegate<?>> DELEGATES_HOLDING_TAG = HashMultimap.create();
    ...
    public static void removeFromAll(Tag tag) {
        ...
        Set<TagDelegate<?>> delegates = new HashSet<>(DELEGATES_HOLDING_TAG.get(tag));
